...
function checkoutDialog(){const lines=cartLines();if(!lines.length)return;const accounts=entries(data.accounts).sort((a,b)=>a[1].customer_name.localeCompare(b[1].customer_name)),hasFood=lines.some(([id])=>category(data.menu[id])==="Comidas"),summary=lines.map(([id,line])=>`<div class="checkout-line"><span>${line.quantity}x ${esc(data.menu[id].name)}</span><b>${money(Number(line.quantity)*Number(data.menu[id].price))}</b></div>`).join("");openModal(`<h2>Finalizar pedido</h2><div class="checkout-summary">${summary}<div class="cart-total"><span>Total</span><b>${money(cartTotal())}</b></div></div>${hasFood?`<div class="notice">Este pedido contém comida e gerará uma única comanda com todos os itens.</div>`:""}<form data-form="checkout"><div class="form-row"><div class="field"><label>Cliente (opcional na venda direta)</label><input name="customer_name" autofocus></div><div class="field"><label>Pagamento</label><select name="payment_method"><option value="">Selecione</option><option>Dinheiro</option><option>Pix</option><option>Cartão</option></select></div></div><div class="field"><label>Observação geral do pedido</label><textarea name="note" placeholder="Ex.: batata sem queijo, entregar na mesa 4..."></textarea></div><label class="account-toggle"><input name="to_account" type="checkbox"> Adicionar à caderneta em vez de registrar venda direta</label><div class="field"><label>Conta existente (ou deixe em Nova conta)</label><select name="account_id"><option value="">Nova conta</option>${accounts.map(([id,a])=>`<option value="${id}">${esc(a.customer_name)}</option>`).join("")}</select></div><button class="primary checkout-button" type="submit">Confirmar pedido · ${money(cartTotal())}</button></form>`)}
...
document.addEventListener("submit",async event=>{
  ...
  try{
    ...
    if(form.dataset.form==="checkout"){
      await mutate({action:"sale.checkout",items:cartLines().map(([menu_id,line])=>({menu_id,quantity:line.quantity})),customer_name:fd.customer_name,payment_method:fd.payment_method,note:fd.note,to_account:form.to_account.checked,account_id:fd.account_id});
