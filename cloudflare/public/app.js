...
function salesPage(){
  ...
  return heading("Saídas da adega")+(!open?`<div class="notice"><b>Não é possível finalizar venda direta com o caixa fechado.</b><br>Entre em Admin → Caixa e abra o caixa. Pedidos adicionados à caderneta continuam disponíveis.</div>`:`<p class="muted">Caixa aberto desde ${date(open[1].opened_at)}.</p>`)+groups;
}
function saleDialog(id){const item=data.menu[id];if(!item)return;const accounts=entries(data.accounts),isFood=category(item)==="Comidas";openModal(`<h2>${esc(item.name)}</h2><p class="price">${money(item.price)}</p>${isFood?`<div class="notice">A comanda deste item será enviada automaticamente para impressão.</div>`:""}<form data-form="sale"><input type="hidden" name="menu_id" value="${id}"><div class="form-row"><div class="field"><label>Quantidade</label><input name="quantity" type="number" min="1" step="1" value="1" inputmode="numeric" required></div><div class="field"><label>Pagamento da venda direta</label><select name="payment_method"><option value="">Selecione</option><option>Dinheiro</option><option>Pix</option><option>Cartão</option></select></div></div><div class="field"><label>Observação</label><input name="note"></div><div class="field"><label>Cliente do pedido (opcional)</label><input name="customer_name"></div><label><input name="send_to_account" type="checkbox" style="width:auto"> Adicionar à caderneta em vez de registrar venda direta</label><div class="field"><label>Conta existente</label><select name="account_id"><option value="">Nova conta</option>${accounts.map(([aid,a])=>`<option value="${aid}">${esc(a.customer_name)}</option>`).join("")}</select></div><div class="actions"><button class="primary" type="submit">Finalizar pedido</button></div></form>`)}
...
document.addEventListener("submit",async event=>{
  ...
  try{
    ...
    if(form.dataset.form==="sale"){
      ...
      const printOrder=category(item)==="Comidas";
      if(form.send_to_account.checked){let aid=fd.account_id;if(!aid){if(!fd.customer_name)throw new Error("Informe o cliente para criar a caderneta.");aid=(await mutate({action:"account.create",customer_name:fd.customer_name,note:""})).id}await mutate({action:"account.addItem",id:aid,description:item.name,price:item.price,quantity:fd.quantity,print_order:printOrder,note:fd.note});}
      else await mutate({action:"sale.create",description:item.name,price:item.price,quantity:fd.quantity,payment_method:fd.payment_method,note:fd.note,customer_name:fd.customer_name,print_order:printOrder});
    }
    closeModal();draw();
  }catch(error){toast(error.message,true)}
});
