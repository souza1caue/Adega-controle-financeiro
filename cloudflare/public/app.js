...
function draw(){
  ...
  if(module==="Frente de caixa"){
    ...
    const content=page==="summary"?summaryPage():page==="menu"?menuPage():page==="cash"?cashPage():page==="accounts"?accountsPage():printerPage();
    app.innerHTML=adminShell(content);
    if(page==="adminPrinter"){refreshTimer=setInterval(()=>load(true).catch(()=>{}),3000);queueAutomaticPrint()}
  }
}

function adminShell(content){const groups=[["Operação",[["summary","Visão geral"],["cash","Controle de caixa"]]],["Cadastros",[["menu","Cardápio"],["accounts","Cadernetas"]]],["Acompanhamento",[["adminPrinter","Central de impressão"]]]];return `<div class="admin-layout"><aside class="admin-sidebar"><div class="admin-identity"><span class="eyebrow">Administração</span><b>Controle da Adega</b></div>${groups.map(([label,items])=>`<div class="admin-nav-group"><small>${label}</small>${items.map(([key,text])=>`<button data-page="${key}" class="${page===key?'nav-active':''}">${text}</button>`).join("")}</div>`).join("")}<button class="ghost admin-logout" data-admin-logout>Sair do Admin</button></aside><section class="admin-content">${content}</section></div>`}
...
function summaryPage(){
  const all=entries(data.sales),nowDate=new Date(),start=new Date(nowDate),end=new Date(nowDate),cashOptions=entries(data.cash).sort((a,b)=>(b[1].opened_at||"").localeCompare(a[1].opened_at||""));let sales=all,periodLabel=summaryFilter;
...
document.addEventListener("submit",async event=>{
  ...
  try{
    ...
    if(form.dataset.form==="summary-range"){
      ...
      summaryFilter=`range:${fd.date_from},${fd.date_to}`;draw();return;
    }
