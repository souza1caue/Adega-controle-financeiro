...
test('category registration preserves measures, balances and optional costs in one batch',async()=>{
  const f=fixture();
  const items=Object.entries(categories).flatMap(([stock_category,category])=>category.units.filter(([unit])=>unit!=='fardo').map(([unit],i)=>({name:`${stock_category} ${unit}`,stock_category,unit,package_quantity:i+0.5,package_cost:i===0?'':2,default_units_per_package:1})));
